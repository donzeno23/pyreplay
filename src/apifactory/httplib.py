...
class HTTPErrorStrategy(object):
    ...
    def handle(self, func):
        ...
        if status_code == http.client.responses[200]: # httplib.OK
            return response
        if status_code == http.client.responses[404]: # httplib.NOT_FOUND
            raise HTTPNotFound()
        if status_code == http.client.responses[400]: # httplib.BAD_REQUEST
...
class HttpMetaSchema(object):
    ...
    def serialize(self, request_data):
        ...
        seq = (get_values(*item) for item in self._get_field_to_keys().items())
    ...
    def deserialize(self, response):
        ...
        def get_values(field):
            ...
            return filter_optional(values, colander.drop)
    ...
    def verify_schema_uniqueness(self):
        ...
        for left, right in itertools.combinations(field_to_keys.values(), 2):
